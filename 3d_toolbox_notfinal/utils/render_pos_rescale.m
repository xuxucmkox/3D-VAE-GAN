function [im, edgemap, pxmap, imt, obj_data, res] = render_pos_rescale(pos, obj_data)
% RENDER_POS render an image using a model and its configuration
    ...
    % is quite slow)
    if (nargin > 1) && isfield(obj_data, 'v')
        v = obj_data.v;
        f = obj_data.f;
        vn = obj_data.vn;
        fn = obj_data.fn;
    else
        [v,f,vn,fn] = readObj_rescale(OBJ3D_DIR, pos.type, pos.obj);
        obj_data.v = v;
        obj_data.f = f;
        obj_data.vn = vn;
        obj_data.fn = fn;
