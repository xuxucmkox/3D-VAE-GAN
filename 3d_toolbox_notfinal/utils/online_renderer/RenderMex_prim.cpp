...
//   mexPrintf("RenderMex\n");
    ...
    double* vertex_interest = mxGetPr(prhs[6]);
    unsigned int num_vertex_interest = mxGetN(prhs[6]);
...
//       }
    ...
    if (0) {
        ...
        glBegin(GL_POINTS);
        for (unsigned int i = 0; i < num_vertex_interest; ++i) {
	  //mexPrintf("%d\n", base_offset+i);
            uint2uchar(base_offset+i,colorBytes);
            glColor3ubv(colorBytes);
            glVertex3dv(vertex_interest);
            vertex_interest+=3;
        }
        glEnd();
    }
    ...
    float* result2 = (float*) mxGetData(plhs[1]);
    glReadPixels(0,0,m_width, m_height, GL_DEPTH_COMPONENT, GL_FLOAT, result2);
     */
