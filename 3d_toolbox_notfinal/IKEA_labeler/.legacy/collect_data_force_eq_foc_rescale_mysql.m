function pos = collect_data_force_eq_foc_rescale(obj_type, reset)
...
% Copyright 2013 Joseph J. Lim
    ...
    if ~reset && exist(filename, 'file') 
        ...
        if strcmp(obj_type, 'ikeaobject_eqfoc')
        ...
        elseif strcmp(obj_type, 'ikearoom_eqfoc')
            s=reshape(textread([TOOLBOX_DATA_DIR 'data_ikearoom2.txt'],'%s'), 8, [])';
        else
...
function gt_info = obtain_gt_info(gt_info, real_id)
    ...
    for foc = 300:10:2000
        ...
        for j = 1:length(gt_info)
            if (1)
                scale = get_scale_factor_hamed(gt_info(j).type, gt_info(j).obj);
            else
                scale = 1;
            end
