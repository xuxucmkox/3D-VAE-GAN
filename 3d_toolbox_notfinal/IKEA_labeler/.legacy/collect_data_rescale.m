...
% Copyright 2013 Joseph J. Lim
    ...
    if ~reset && exist(filename, 'file') 
        ...
        for i = 2:size(s,1)
            ...
            if (1)
                scale = get_scale_factor_hamed(r{3}, r{4});
            else
                scale = 1;
            end
            [bestRp, bestTp, bestFoc, bestErr, v2d, v3d, A] = render_helper(r{2}, regexp(r{5}, '::', 'split'), scale);
...
function [bestRp, bestTp, bestFoc, bestErr, v2d, v3d, A2] = render_helper(real_id, matched_particle, scale)
